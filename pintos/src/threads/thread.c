...
static int load_avg;           /**< Recent average number of ready threads. */
...
/** calc priority based on MLFQS */
void thread_priority_calc_mlfqs(struct thread *t, void *aux UNUSED)
{
  if (t == idle_thread)
    return;
  int new_priority = FP_TO_INT_TRUNC(
      FP_SUB(FP_SUB(FP_FROM_INT(PRI_MAX), FP_DIV_INT(t->recent_cpu, 4)),
...
/** calc recent_cpu based on MLFQS */
void thread_recent_cpu_calc_mlfqs(struct thread *t, void *aux UNUSED)
{
  if (t == idle_thread)
    return;
...
/** calc load_avg based on MLFQS */
void load_avg_calc_mlfqs(void)
{
...
void thread_recent_cpu_incre(struct thread *t)
{
  if (t == idle_thread)
    return;
...
void thread_set_priority(int new_priority)
{
  ...
  donate_priority_update(cur);
  if (!list_empty(&ready_list))
  {
    struct thread *nextt = list_entry(list_begin(&ready_list), struct thread, elem);
...
void thread_set_nice(int nice)
{
  ...
  struct thread *cur = thread_current();
  cur->nice = nice;
  thread_priority_calc_mlfqs(cur, NULL);
  if (!list_empty(&ready_list))
  {
    struct thread *nextt = list_entry(list_begin(&ready_list), struct thread, elem);
    if (nextt->priority > cur->priority)
...
{
  enum intr_level old_level = intr_disable();
  int result = FP_TO_INT_ROUND(FP_MUL_INT(load_avg, 100));
  intr_set_level(old_level);
  return result;
}
...
{
  enum intr_level old_level = intr_disable();
  int result = FP_TO_INT_ROUND(FP_MUL_INT(thread_current()->recent_cpu, 100));
  intr_set_level(old_level);
  return result;
}
...
#ifdef USERPROG /*初始化 Lab2 相关的 fdtable 和 childinfo*/
