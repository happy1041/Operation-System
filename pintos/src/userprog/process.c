...
#ifdef VM
    if (!page_register_file(&thread_current()->spt, upage, file, ofs,
...
setup_stack(void **esp, const char *cmd_line)
{
  ...
  if (!page_register_stack(&thread_current()->spt, stack_page, true))
  {
    frame_free(kpage);
    return false;
  }
  ...
  {
    page_unregister(&thread_current()->spt, stack_page);
    frame_free(kpage);
    return false;
  }
#else
