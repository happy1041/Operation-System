 ...
 *
 * 设计目标：
 *   父进程和子进程都需要访问子进程的退出状态，但两者的生命周期相互独立——
 *   子进程可能先于父进程退出，父进程也可能先于子进程退出。
 *   因此这个结构体在堆上分配，用 ref_count 实现共享所有权：
 *     - 初始 ref_count = 2（父、子各持有一份引用）；
 *     - 任意一方退出时将 ref_count 减 1；
 *     - 最后一个退出的一方负责 free 掉这块内存。
 *
 * 同步机制：
